        ...
        new Thread() {
            ...
            public void run() {
                ...
                try {
                    sleep(3000);
                    File file = new File(Environment.getExternalStorageDirectory(), "aa.mp4");
                    File outFile = new File(Environment.getExternalStorageDirectory(), "cc.pcm");
                    if (file.exists()) {
                        String input = file.getAbsolutePath();
                        Log.e("file", input);
                        String out = outFile.getAbsolutePath();
                        Log.e("file", out);
                        VideoPlayer.getInstance().sound(input, out);
                    } else {
                        Log.e("file", "file no exists :" + file.getAbsolutePath());
                    }
                } catch (InterruptedException e) {
            ...
            public void run() {
                ...
                try {
                    ...
                    if (file.exists()) {
                        ...
                        String out = outFile.getAbsolutePath();
                        Log.e("file", out);
                        VideoPlayer.getInstance().soundAudioTrack(input, out);
                    } else {
                        Log.e("file", "file no exists :" + file.getAbsolutePath());
                    }
                } catch (InterruptedException e) {
